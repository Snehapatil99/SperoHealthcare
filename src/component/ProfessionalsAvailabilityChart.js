...
export default function ProfessionalsAvailabilityChart() {
  ...
  useEffect(() => {
    axios
      .get('http://122.176.232.35:8008/web/professional-count/')
      .then((response) => {
        const { assigned_professionals, unassigned_professionals } = response.data;
  ...
  return (
    <>
      ...
      <Grid container spacing={1} sx={{ marginBottom: 1 }}>
        <Grid item xs={6}>
          <Card sx={{ backgroundColor: '#fbbd0', border: '1px solid #000', borderRadius: '10px' }}>
      ...
      <Card sx={{ backgroundColor: '#fbbd0', border: '1px solid #000', padding: '8px', borderRadius: '10px' }}>
        <CardContent sx={{ paddingBottom: 1 }}>
          <Typography variant="h6" color="primary" align="center" sx={{ marginBottom: '8px' }}>
            PROFESSIONALS AVAILABILITY
          </Typography>
          <Doughnut data={chartData} />

          <Box mt={1} align="center">
            <Typography variant="body2" sx={{ color: '#000', marginBottom: '4px' }}>
              Total Professionals: {assignedProfessionals.total + unassignedProfessionals.total}
            </Typography>

            <Box display="flex" justifyContent="center" alignItems="center">
              <Box sx={{ display: 'flex', alignItems: 'center', marginRight: '12px' }}>
                <Box sx={{ width: 10, height: 10, backgroundColor: '#42A5F5', borderRadius: '50%', marginRight: '4px' }} />
                <Typography variant="body2" sx={{ color: '#000' }}>Assigned</Typography>
                <Typography variant="h6" sx={{ color: '#000' ,marginLeft:"10px" }}>{assignedProfessionals.total}</Typography>

              </Box>
              <Box sx={{ display: 'flex', alignItems: 'center' }}>
                <Box sx={{ width: 10, height: 10, backgroundColor: '#FF7043', borderRadius: '50%', marginRight: '4px' }} />
                <Typography variant="body2" sx={{ color: '#000' }}>Unassigned</Typography>
                <Typography variant="h6" sx={{ color: '#000', marginLeft:"10px" }}>{unassignedProfessionals.total}</Typography>
